...
import '../../../components/styles/AddMedicine.css';
...
const AddMedicine = () => {
  ...
  const handleAddMedicine = async (e) => {
    ...
    if (!name || !genericName || !dosageForm || !strength || !price || !strip || !manufacturer) {
      const valMsg = 'Please fill all required fields';
      setError(valMsg);
      toast.error(valMsg);
      setSuccess('');
      return;
    ...
    try {
      ...
      setImage(null);
      setImageFileName('');
      const succMsg = 'Medicine added to the list successfully!';
      setSuccess(succMsg);
      toast.success(succMsg, { title: 'Medicine Added' });
      setError('');
    } catch (err) {
      const errMsg = 'Failed to add medicine to the list.';
      setError(errMsg);
      toast.error(errMsg);
      setSuccess('');
    }
