...
const TestService = () => {
  ...
  const handleSubmit = async () => {
    if (!patientName || !patientEmail || !phoneNumber) {
      const vMsg = 'Please enter patient name, email, and phone number';
      setError(vMsg);
      toast.error(vMsg);
      return;
    }
    if (selectedItems.length === 0) {
      const noItemsMsg = 'Please select at least one test or service';
      setError(noItemsMsg);
      toast.error(noItemsMsg);
      return;
    }
    try {
      const billData = {
      ...
      const succ = 'Bill sent to patient successfully!';
      setSuccess(succ);
      toast.success(succ, { title: 'Billing Dispatched' });
      setError('');
      ...
      console.error(err);
      const errMsg = 'Failed to submit bill.';
      setError(errMsg);
      toast.error(errMsg);
